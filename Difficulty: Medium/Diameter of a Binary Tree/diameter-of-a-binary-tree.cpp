...
class Solution {
  public:
    ...
    if (!root) return {0, 0}; // Base case: height = 0, diameter = 0
    ...
    // Max diameter considering left, right, or root path
    int currDiameter = max(rootDiameter, max(left.second, right.second));
