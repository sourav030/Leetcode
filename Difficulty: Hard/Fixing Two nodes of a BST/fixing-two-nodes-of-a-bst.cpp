...
class Solution {
  public:
    void makeBst(Node* root,vector<int>&ans,int &index){
        if(!root) return;
        makeBst(root->left,ans,index);
        root->data=ans[index++];
        makeBst(root->right,ans,index);
    }
    void inordere(Node* root, vector<int> &ans){
        if(!root) return;
